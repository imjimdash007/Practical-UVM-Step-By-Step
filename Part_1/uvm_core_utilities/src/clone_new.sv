...
module top;
  ...
  initial begin
    // Instantiate with only one argument (name) since it's a uvm_object
    class_A_inst1 = new("child_inst1");

    // void'() suppresses the Questasim warning about ignoring the return value
    void'(class_A_inst1.randomize());
    
    class_A_inst1.print();
    $cast(class_A_inst2, class_A_inst1.clone());
    class_A_inst2.print(); // Changed this to print inst2 to prove clone worked
  end 
