...
class class_A extends uvm_object;
  ...
  // FIX: Added 'init_val' with a default of 10 so it accepts a 2nd argument
  function new(string name="", int init_val=10);
    int idx = 1;
    super.new(name);	
    cl_string = name;
    
    // FIX: Initialize cl_int with the passed argument instead of hardcoded 10
    cl_int = init_val; 
...
class class_B extends uvm_object;
  ...
  // Objects to demonstrate the copy recursion policy
  ...
  function new(string name="");
    ...
    // FIX: This now works perfectly because class_A accepts the 2nd argument
    cl1 = new(name, 10);
    cl3 = new(name, 11);
  ...
  virtual function bit do_compare(uvm_object rhs, uvm_comparer comparer);
    ...
    // FIX: The author accidentally compared par_int here. Changed to par_address.
    do_compare &= comparer.compare_field_int("par_address", par_address, rhs_.par_address, 8, UVM_NORADIX);
    ...
    // FIX: The author accidentally labeled this string "cl1". Changed to "cl3".
